...
def start(update, _):
    ...
    update.message.reply_text(
        'Меня зовут Барни Бот. Давай пообщаемся! '
...
def gender(update, _):
    ...
    update.message.reply_text(
        'Хорошо. Пришли мне свое фото, чтобы я знал, как ты '
...
def skip_location(update, _):
    ...
    update.message.reply_text(
        'Точно параноик! Ну ладно, тогда расскажи мне что-нибудь интересное...'
    )
    # переходим к этапу `BIO`
    return BIO
...
if __name__ == '__main__':
    ...
    conv_handler = ConversationHandler( # здесь строится логика разговора
        ...
        states={
            GENDER: [MessageHandler(Filters.regex('^(Парень|Девушка)$'), gender)],
